...
공격시 HP와 데미지 처리 관련 방법은 여러가지있다.

1. 공격하는 당사자에서 HP 감소 처리.
2. 공격당하는 당사자에서 HP 감소 처리.
3. 제 3의 관계자인 매니저에서 HP 감소 처리

등이 있는데 보통의 게임들은 2번 로직을 따른다 왜냐하면 당사자가 무적 상태이거나 혹은 버프를 걸었거나 또는 아이템등 갖가지 기능을 갖고 있기 때문에 해당 로직을 처리하기 가장 적합하기 때문이다.

## 몬스터 자동 리스폰

보통은 서버 측에서 한다. 몬스터가 죽으면 몬스터의 전체적인 숫자를 맞춰주기 위해 랜덤한 위치에서 스폰을 한다. 이렇게 서버측에서 스폰을 한 후 클라이언트에게 알려준다.
